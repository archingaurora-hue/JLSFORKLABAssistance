...
if (isset($_POST['save_employee'])) {
    ...
    if (empty($id)) {
        ...
        // Save to DB
        $stmt = $conn->prepare("INSERT INTO `User` (first_name, last_name, email, password, role, created_at) VALUES (?, ?, ?, ?, ?, NOW())");
        $stmt->bind_param("sssss", $first_name, $last_name, $email, $hashed_pwd, $role);
        ...
        if (!empty($raw_password)) {
            ...
            $hashed_pwd = password_hash($raw_password, PASSWORD_DEFAULT);
            $stmt = $conn->prepare("UPDATE `User` SET first_name=?, last_name=?, email=?, password=? WHERE user_id=? AND role='Employee'");
            $stmt->bind_param("ssssi", $first_name, $last_name, $email, $hashed_pwd, $id);
        } else {
            // Update info only
            $stmt = $conn->prepare("UPDATE `User` SET first_name=?, last_name=?, email=? WHERE user_id=? AND role='Employee'");
