...
// Modified Query: Removed the strict WHERE clause to allow Completed and Cancelled orders to appear,
// and changed to LEFT JOIN in case an order exists but doesn't have bags yet.
...
// 1. First, group raw data by order_id
$groupedOrders = [];
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        // Only add load if load_id is not null (due to LEFT JOIN)
...
// 2. Second, bucket the grouped orders by their Master Status
$pendingOrders = [];
$inProgressOrders = [];
$completedOrders = [];
$cancelledOrders = [];
...
// Prepare arrays for the Accordion UI
$orderGroups = [
